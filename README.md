...
<p align="center">
  <a href="https://star-history.com/#Vit129/graphify&Date">
...
This fork started from [safishamsi/graphify](https://github.com/safishamsi/graphify) and has repeatedly adopted upstream's own `v8` rewrite branch as its base over several months (most recently 2026-07-02) — the shared foundation (`affected.py`, `resolver_registry.py`, `symbol_resolution.py`, and most of the rest of the package) is upstream's own code, not this fork's invention. Checked directly against upstream's current source (`3140b2e`, v0.9.6), not assumed — the real, current, standing diff is on top of that shared base and is narrower than earlier versions of this section claimed:

- **A real query layer, genuinely fork-only.** Upstream's `graphify/serve.py` still resolves nodes with the older hand-rolled tiered scorer (`_EXACT_MATCH_BONUS = 1000.0` — exact/prefix/substring bonuses, no BM25) and a bare `nx.shortest_path(G, src, tgt)` for `shortest_path` — no hub penalty, and only a single top-scored candidate pair is ever tried (it does warn on close ties, but doesn't retry). This fork's `graphify/query.py` (built in one session, kept over upstream's copy in the last merge specifically because it supersedes it) adds real BM25 scoring, typo/synonym-corrected retry, and `find_path_with_disambiguation` — which retries every near-tied candidate pair and explicitly avoids degree-heavy hub nodes before falling back to one. `--path`/`--source-path`/`--target-path` flags on `path`/`explain` are part of the same fork-only layer.
- **Language/format extraction breadth.** Upstream supports 36 tree-sitter grammars; this fork supports 54 — `.css`/`.scss`, `.html`/`.htm`, `.yaml`/`.yml`, `.toml`, `.robot`/`.resource`, `.feature` (Gherkin), and Fish shell have no extractor anywhere in upstream (checked directly, not inline either).
- **Opt-in config value-coupling.** A schema-agnostic heuristic (`shares_value:<value>` edges, gated behind `value_coupling = true`) connects files that reference the same identifier-shaped value (e.g. YAML `entity_id`s) — confirmed absent from upstream entirely.
