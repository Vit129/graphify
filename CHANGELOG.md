...
- Fix: `path` now tries every near-tied candidate on each side (within 10% of the top score) before reporting no path found, instead of only the single arbitrary top-of-tie pick. Found dogfooding a bilingual content repo: a duplicate section heading ("Stats") repeated across parallel English/Japanese files meant the wrong tied candidate could be picked, silently reporting "no path found" even when a different tied candidate WAS reachable. The ambiguity warning now also lists every candidate node ID considered, not just a runner-up score.
- Fix: `_SKIP_FILES` (graphify's own generated report filenames, never treated as source input) now matches by prefix (`GRAPH_REPORT*`/`GRAPH_SUMMARY*`), not exact name - a hand-made backup like `GRAPH_REPORT_BACKUP.md` was still exact-name-escaping the check and getting re-ingested, with its own "## Suggested Questions" heading becoming a competing node in the real graph. Found dogfooding a real repo with such a backup file.
- Docs: `explain "<concept>"` wording (in every always-on skill instruction file, across all 6 platforms) overpromised free-form semantic concept search - the underlying match (`_find_node_core`) is exact/prefix/substring against a node's label or file path, not a concept lookup. Two independent dogfooding sessions across different projects were misled by this framing and got empty/wrong results trying to `explain` a phrase instead of a name. Reworded to `explain "<ClassName/FileName>"` with an explicit "name match, not free-form concept search - use `query` for that" note.
...
- Add self-update check: `graphify` now compares the running version against PyPI once every 24h (skipped for `install`/`uninstall`/`hook-check`/`--version`/`--help` so those stay instant and CI never triggers a network call) and, when `autoUpdate` isn't explicitly disabled in `~/.config/graphify/config.json`, kicks off `uv tool upgrade graphifyy` (falling back to `pipx upgrade`) as a detached background process — mirrors the same bootstrap-once-then-hands-off pattern already shipped in `agy-plugin-cc`/`agy-plugin-codex`. New `graphify/update_check.py`; wired into `__main__.py`'s existing per-invocation skill-version-check block.

## 0.12.0 (2026-07-02)

- Merged upstream through `Vit129/graphify@v8`'s 0.9.5 (see that section below) into this fork's `main`. One real overlap with this fork's own P1/P9 query-pipeline work: upstream independently fixed the same "question words dominate BFS seeding" bug this fork's P1 reopen already addressed, with a broader stopword list (`why`/`when`/`where`/`has`/`have`/`work`/`works`/`working`/etc., not in this fork's own list) and a fallback-to-unfiltered behavior for all-stopword queries (`"how does it work"` now seeds on `how`/`does`/`work` instead of nothing) that this fork's `_query_terms` didn't have. Merged both stopword lists into one in `graphify/query.py`'s `_STOPWORDS` and ported the fallback logic in; this fork's version lives in `query.py` (not `serve.py`, where upstream's fix landed) since P1's session already relocated the whole query pipeline there — `serve.py`'s conflicting standalone copy of the pre-BM25 scoring logic was discarded in favor of this fork's BM25 rewrite, which already supersedes it.
...
- Feat: `query_graph`/`get_node`/`get_neighbors`/`shortest_path` now recover from typos and abbreviations when the primary lexical search finds nothing. A failing query term is spell-corrected against the graph's own identifier vocabulary (Damerau-Levenshtein for typos like `sesion`->`session` or `recieve`->`receive`; ordered-subsequence matching for abbreviations like `hus`->`handleUserSession`) and the corrected term is re-run through the normal, unmodified scoring pipeline — not a second scoring algorithm, so it inherits IDF weighting and camelCase sub-word matching automatically. Only triggers when the uncorrected query returns zero results, so a query that already matches pays no extra cost. The response always states when a correction was applied (`Note: no exact match; corrected possible typo(s) "sesion" -> "session"`) rather than silently substituting. Validated on a real 14,838-node graph: a query with three simultaneous typos in one phrase resolved all three and surfaced the correct target as the top result.
- Fix: search tokenizer now splits camelCase/snake_case/kebab-case identifiers into sub-words (`getUserData` -> `get`, `user`, `data`; `HTTPServer` -> `HTTP`, `Server`, acronym runs preserved). Previously `_search_tokens` used a plain `\w+` split, so a multi-word natural-language query could never reach the exact/prefix scoring tier against a camelCase or snake_case label — it was permanently capped at the weak substring tier, regardless of match quality. A sub-word exact match now scores at the prefix tier (not promoted all the way to exact, which stays reserved for whole-label matches). Validated against a real project graph: querying "reconcile session persistence" previously didn't surface `.reconcileSessionPersistenceWithMode()` in the top 5 results at all; it now ranks #1.
...
## 0.9.5 (2026-07-02, merged from upstream)
