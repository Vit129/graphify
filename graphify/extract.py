...
def _resolve_js_import_target(raw: str, str_path: str) -> "tuple[str, Path | None] | None":
    ...
    # path). Namespace the id with the "ref" prefix - the same convention used
    # for tsconfig `extends`/`$ref` externals - so it can NEVER collapse to the
    # same _make_id as a local file/symbol node. Without it, the bare
...
def _extract_generic(
    ...
    # C# receiver typing is method-scoped (#2299): class fields/properties
    ...
    def walk_calls(
        ...
        if node.type in config.call_types:
            ...
            if callee_name and callee_name not in _LANGUAGE_BUILTIN_GLOBALS:
                ...
                # C#: any member call (`recv.Method()`) must defer to the cross-file
