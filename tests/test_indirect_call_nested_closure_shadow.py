"""JS/TS closure-body descent with parameter and local shadowing (#2241).

Descend into previously-unwalked JS/TS closure bodies to capture indirect_call
references attributed to the enclosing caller (previously closures were fully
opaque to indirect_call resolution). To prevent false positives from this
new descent, the closure's own parameters and locals widen the shadow set for
calls made inside that closure's subtree only, ensuring a local identifier
(e.g. `rows.map((r) => c.get(r))`) does not bind to an unrelated same-named
callable elsewhere in the corpus.

These tests verify that closure descent correctly attributes unshadowed genuine
references while properly shadowing closure parameters and locals.

Out of scope here: a `for (const x of xs)` loop variable not wrapped in a
`variable_declarator` is a separate gap in the same shadow-set computation,
addressed by #2568 — not retested here to avoid overlapping that diff.
"""
import os
