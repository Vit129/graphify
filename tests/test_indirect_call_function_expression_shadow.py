"""Function expressions (including generator FEs and TSX) must shadow indirect_call args.

An inline arrow was already covered by #2241: `walk_calls` descends into an
untracked closure and folds its own parameter/local bindings into `extra_locals`,
preventing them from reading as unresolved by-name references to same-named
callables elsewhere in the corpus.

A function expression (`function (k) {…}`) or generator function expression
(`function*(k){…}`) must shadow in the exact same way.
"""
import os
